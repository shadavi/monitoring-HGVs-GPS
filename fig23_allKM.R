...
cnt_km <- ldply(file_ids, function(fid) {
  print(fid)
  ...
  ldply(sub_files, function(fname, fid) {
    if (file.exists(fname)) {
      ...
      data.frame(time=trunc(obu$start_t[1], "hour"), count=sum(obu$obs_dist_driven))
    }
  }, fid)
})
...
cnt_km_week<-cnt_km[!(weekdays(cnt_km$time) %in% weekdays_exclude), ]

# run quantile function over splits of data per hour
cnt_km_week$hour <- hour(cnt_km_week$time)
plotdata <- ddply(cnt_km_week, .(hour), function(x) quantile(x$count, c(0.05,0.25,0.5,0.75,0.95)))

colnames(plotdata)	<-	c("hour","lower","q25","median","q75","upper") 
plot_quantiles(plotdata,"","Total driven \ndistance(km)")
dev.copy2pdf(file='/home/sheida/Workplace/papers/obu_paper/images/23.pdf',out.type="cairo", width=10.1, height=4)
